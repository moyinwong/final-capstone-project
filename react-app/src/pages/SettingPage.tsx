...
const SettingPage = () => {
  ...
  let getUserInfo = async () => {
    ...
    if (user.stripeId) {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/payment/check-stripe-account-status/${user.email}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    let firstNameField = document.getElementById(
      "firstName"
    ) as HTMLInputElement;
    if (firstNameField) {
    ...
    let introductionField = document.getElementById(
      "introduction"
    ) as HTMLInputElement;
  ...
  let submitHandler = async (
    ...
    if (firstName.length === 0) {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/user/edit-profile/${userId}`,
        {
          method: "PUT",
          body: formData,
        }
  ...
  return (
    <Container id="setting-container" component="main" maxWidth="md">
      ...
      <div className={classes.paper}>
        ...
        <div>
          {profilePicture.match(/http/) ? (
            <Image id="edit-user-image" src={profilePicture}></Image>
          ) : (
            <Image
              id="edit-user-image"
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                helperText={
                  isEmpty
                    ? "密碼長度至少為8個字元，並至少包含1個數字、1個大寫英文字母、1個小寫英文字母"
                    : ""
                }
                variant="outlined"
                required
