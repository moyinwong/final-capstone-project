...
export default function LoginPage (state: {
  ...
  let handleEmailChange = (event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    setIsError(false)
    setEmail(event.target.value)
  }

  let handlePassWordChange = (event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
    setIsEmpty(false)
  ...
  let submitHandler = (event: React.MouseEvent<HTMLElement, MouseEvent>) => {
    ...
    if (!email.match(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/g)) {
      ...
      dispatch(
        login(email, password, previousLocation)
      );
    }
  }
