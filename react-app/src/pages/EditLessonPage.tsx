...
const EditLessonPage = () => {
  ...
  const getFiles = async () => {
    ...
    let retrievedFiles = result.files.map((file: any) => (
      <li key={file.file_name}>{file.file_name}</li>
    ));
  ...
  useEffect(() => {
    let lessonTitleField = document.getElementById(
      "lessonTitle"
    ) as HTMLInputElement;
    if (lessonTitleField) {
      lessonTitleField.value = lessonTitle;
    }
    let isTrialField = document.getElementById(
      "demo-simple-select"
    ) as HTMLInputElement;
    if (isTrialField) {
      isTrialField.value = isTrial;
    }
    let descriptionField = document.getElementById(
      "lessonDescription"
    ) as HTMLInputElement;
    if (descriptionField) {
      descriptionField.value = description;
    }
    let videoURLField = document.getElementById(
      "lessonVideoUrl"
    ) as HTMLInputElement;
    if (videoURLField) {
    ...
    <div>
      <Container id="edit-lesson-container" component="main" maxWidth="md">
        ...
        <form id="edit-lesson-form" noValidate autoComplete="off">
          ...
          {completed && (
            <div>
              成功建立課堂<i className="fas fa-check-circle"></i>
            </div>
          )}
        </form>
      </Container>
